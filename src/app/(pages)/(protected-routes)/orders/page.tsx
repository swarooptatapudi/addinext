...
export default function Orders(): React.JSX.Element {
  ...
  // ✅ MOVE ALL HANDLERS OUTSIDE COLUMNS DEFINITION
  ...
  const handlePayment = useCallback(async (order: Order) => {
    try {
      ...
      // ✅ Refetch orders after payment to update status
      refetch();
    } catch (err) {
      console.error('Payment initiation failed:', err);
      toast.error('Failed to process payment.');
    }
  }, [getOrderDetails, startPayment, refetch]);

  const handleOrderIdClick = useCallback((order: Order) => {
    ...
    if (route) {
      router.push(`${route}?${new URLSearchParams(baseParams).toString()}`);
    }
  }, [router]);
  ...
  // ✅ MOVE handleDesignClick OUTSIDE columns
  const handleDesignClick = useCallback(async (order: Order) => {
    try {
  ...
  ], [handleOrderIdClick, handlePayment, handleDesignClick, handleExport, isPaymentLoading, exportingIds]);
