...
export default function Orders(): React.JSX.Element {
  ...
  const handleOrderIdClick = (order: Order) => {
    ...
    const pathMap: Record<string, string> = {
      'BK Orders': 'BK',
      'Insole Orders': 'Insoles',
      'Cranial Helmet Orders': 'Cranial',
    };

    const path = pathMap[order.device_type];
    if (path) router.push(`/orders/new-order/${path}?${query}`);
  };
        ...
        return (
          <span
            className={`px-2 py-1 rounded-full text-xs ${
              statusMap[row.original.status] || 'bg-gray-100 text-gray-800'
            }`}
          >
            {row.original.status}
          </span>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const canPay =
          order.order_value > 0 &&
          !hasPayment &&
          !['paid', 'completed', 'cancelled'].includes(normalizedStatus);
