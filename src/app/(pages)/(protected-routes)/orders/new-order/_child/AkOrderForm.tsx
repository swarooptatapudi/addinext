...
const WatchFieldReset = () => {
  ...
  const prevSocket = React.useRef(values.socket_type);

  useEffect(() => {
    // First load → do nothing
    if (!prevSocket.current) {
      prevSocket.current = values.socket_type;
      return;
    }

    // Only reset if USER changes socket
    if (prevSocket.current !== values.socket_type) {
...
export default function AkOrderForm({ item_type }: { item_type: string }): React.JSX.Element {
  ...
  const [createOrder, { isLoading: isOrderCreating, isSuccess }] = useCreateAKOrderMutation();
      ...
      getOrderDetails({
        ...
        .then((response) => {
          ...
          const formattedData = {
            ...
            height: apiData.height ? String(apiData.height) : '',
            weight: apiData.weight ? String(apiData.weight) : '',

            // Order config
            socket_type: apiData.socket_type || '',
            design_variation: apiData.design_variation || '',
            model_name: apiData.model_name || '',
            activity_level: apiData.activity_level || '',

            // Measurements
            stump_length: apiData.stump_length
              ? String(apiData.stump_length)
              : '',

            mpt_distance: apiData.mpt_distance
              ? String(apiData.mpt_distance)
              : '',

            floor_distance: apiData.floor_distance
              ? String(apiData.floor_distance)
              : '',

            waist_circumference: apiData.waist_circumference
              ? String(apiData.waist_circumference)
              : '',

            Foot_length: apiData.foot_length
              ? String(apiData.foot_length)
              : '',

            shoe_size: apiData.shoe_size
              ? String(apiData.shoe_size)
              : '',

            // Angles (IMPORTANT FIX)
            flexion_angle: apiData.flexion_angle
              ? String(apiData.flexion_angle)
              : '',

            abductionadduction_angle:
              apiData.abduction_angle || apiData.adduction_angle
                ? String(
                  Number(apiData.abduction_angle || 0) +
                  Number(apiData.adduction_angle || 0)
                )
                : '',

            // Extra fields
            stump_type: apiData.stump_type || '',
            stump_condition: apiData.stump_condition || '',
            previous_prosthetic_experience:
              apiData.previous_prosthetic_experience || '',

            // Arrays
            ak_socket_measurements: Array.isArray(apiData.ak_socket_measurements)
              ? apiData.ak_socket_measurements.map((item: any) => ({
                ...item,
                measurement_cm: item.measurement_cm
                  ? String(item.measurement_cm)
                  : '',
                desired_reduction_: item.desired_reduction_
                  ? String(item.desired_reduction_)
                  : '',
              }))
              : [],

            table_zbib: Array.isArray(apiData.table_zbib)
              ? apiData.table_zbib.map((item: any) => ({
                ...item,
                pressure_mm: item.pressure_mm
                  ? String(item.pressure_mm)
                  : '',
              }))
              : [],
          };
          ...
          if (apiData.item_code) {
            setSelectedItem(apiData.item_code);
          }
        })
    .catch((err) => {
  ...
  if (orderId && !isInitialDataLoaded) {
          ...
          <div className="flex flex-col gap-6">
            ...
            {currentStep === 5 && (
              <AKFinish
                ...
                isActiveStep={currentStep === 5} // ✅ ADD THIS
              />
            )}
