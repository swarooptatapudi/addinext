...
import { useGetItemNameAfoByDetailsMutation } from '@/rtk-query/apis/products';
...
export default function AFOOrderForm({ item_type }: { item_type: string }): React.JSX.Element {
  const { data, isLoading: isFormOptionsLoading } = useGetFormSettingsQuery('Insoles');
  const [createAfoOrder, { isLoading: isOrderCreating, isSuccess }] =
    useCreateAfoOrderMutation();
  const { user }: { user: USER } = useSelector((state: any) => state.userReducer);
  // console.log(" Logged-in user from Redux:", user);
  const [selectedItem, setSelectedItem] = React.useState<string>('');
  const [getItem, { isLoading: isItemFetching }] = useGetItemNameAfoByDetailsMutation();
