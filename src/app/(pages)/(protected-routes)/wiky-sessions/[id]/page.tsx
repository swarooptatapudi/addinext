...
export default function WikySessionWorkspacePage() {
  ...
  async function loadSession() {
    setError(null);
    const r = await fetch(`/api/method/addiwise.apis.wiky_sessions.get?name=${id}`);
  ...
  async function loadFiles() {
    ...
    try {
      ...
      const r = await fetch(
        `/api/method/addiwise.apis.wiky_sessions.list_files?session_id=${id}`
      );
      const d = await r.json();
      setFiles(d?.message || []);
    } catch {
  ...
  async function refreshSession() {
    ...
    try {
      // Refresh/login, update token + expiry in Wiky Scan Session
      await fetch('/api/method/addiwise.apis.wiky_auth.login', {
        method: 'POST',
  ...
  function launchDesign() {
    ...
    router.push(`/wiky-sessions/${id}/design`);
  }

  async function markCompleted() {
    setActionLoading(true);
    try {
      await fetch('/api/method/addiwise.apis.wiky_sessions.mark_completed', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ session_id: id }),
      });
  ...
  if (loading) {
    return <div className="p-6 text-gray-500">Loading Wiky Workspace…</div>;
  }
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Top band */}
      <div className={`rounded-lg px-4 py-3 text-white flex items-center justify-between ${bandClass}`}>
        ...
        <div className="flex gap-4 text-sm">
          ...
          <button onClick={() => router.push('/wiky-sessions')} className="underline">
            Back
          </button>
        </div>
      </div>
      ...
      <div className="border rounded-lg bg-white p-4">
        <div className="text-lg font-semibold mb-3">Wiky Design Workspace</div>
      ...
      <div className="border rounded-lg bg-white">
        <div className="flex items-center justify-between px-4 py-3 border-b">
          <div className="font-medium">Wiky Files</div>
          <button
            onClick={loadFiles}
        ...
        {!filesLoading && files.length === 0 && (
          <div className="px-4 py-8 text-center text-gray-500">
            No files available in Wiky yet.
