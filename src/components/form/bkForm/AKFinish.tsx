...
import { BookmarkIcon, Check, Loader, X, CoinsIcon } from 'lucide-react';
...
export const AKFinish = ({
  ...
  const [selectedSubColor, setSelectedSubColor]       = useState<{ label: string; hex: string } | null>(null);
  ...
  // ✅ FIX: was always true due to truthy string literal
  const showFinishOptions =
    isAddiEase ||
    isAddiEaseEco ||
    isAddiEaseL ||
    values.model_name === 'AddiEaseMould' ||
    values.model_name === 'AddiEaseMould-HR';
  ...
  useEffect(() => {
    ...
    // ✅ Removed duplicate useEffect that also force-set these below
  }, [formSettings, setFieldValue]);

  // ─── Finish / Latices defaults ────────────────────────────────────────────
  useEffect(() => {
    ...
    if (orderId && deviceTypeId) {
      ...
      if ((showFinishOptions && !values.finish_type) || initialLoad) {
        if (isAddiEase || isAddiEaseL || isAddiEaseEco) {
    ...
    isDesignSelf, values.model_name, showFinishOptions,
    isAddiEase, isAddiEaseL, isAddiEaseEco,
    setFieldValue, initialLoad, orderId, deviceTypeId
  ]);
  ...
  useEffect(() => {
    if (showEstimateCard && !initialLoad) {
      ...
      const hasChanged = relevantFields.some((f) => values[f] !== prevValues[f]);
      if (hasChanged) {
  ...
  useEffect(() => {
    const fetchColors = async () => {
      try {
        const data = await getProductColorStep5({}).unwrap();
        setSubColors(data.map((name: string) => ({
          label: name,
          hex: getTailwindColorClass(name)
        })));
      } catch (err) {
  ...
  const debouncedCouponValidation = useCallback(() => {
    ...
    if (couponCode.trim().length < 5) { setCouponData(null); return; }
    const timer = setTimeout(() => handleCouponValidation(), 1000);
    setCouponTimeout(timer);
    return () => { if (couponTimeout) clearTimeout(couponTimeout); };
  }, [couponCode]);
  ...
  const getTailwindColorClass = (name: string) => {
    const colorMap: Record<string, string> = {
      ...
      green: '#10B981', yellow: '#FACC15', pink: '#EC4899',
      brown: '#78350F', orange: '#FB923C', purple: '#8B5CF6', gray: '#9CA3AF'
    };
    return colorMap[name.toLowerCase()] || '#888888';
  };

  const validateBeforeAction = () => {
    if (!selectedItem)      { toast.error('Please complete the basic form first'); return false; }
    if (!values.design_by)  { toast.error('Please select Design by'); return false; }
    if (!values.print_by)   { toast.error('Please select Print by'); return false; }
    return true;
  };

  const getBasePriceLabel = () => {
    const dA = values.design_by === 'Addiwise';
    const pA = values.print_by  === 'Addiwise';
    const dS = values.design_by === 'Self';
    const pS = values.print_by  === 'Self';
    if (dA && pA) return 'Design + Print';
    if (dA)       return 'Design';
    if (pA)       return 'Print';
    if (dS && pS) return '';
    if (dS)       return 'Self Design';
    if (pS)       return 'Self Print';
    return 'Base';
  };

  const getFinishOptions = () => {
    if (isAddiEase || isAddiEaseL || isAddiEaseEco) {
      return [{ value: 'Dye', label: 'Black Dye', color: 'bg-black text-white' }];
    }
    return [];
  };
  ...
  const handleCouponValidation = async () => {
    if (!couponCode.trim() || couponCode.trim().length < 5) { setCouponData(null); return; }
  ...
  const handleEstimateClick = async () => {
    ...
    try {
      ...
      // ✅ Parent display state only
      setTotalDiscount(data.total_distcounted_price);
      setDesgin(data.design);
      setPrint(data.print);
      setCouponPer(data.additional_discount);
      setTotalPrice(data.total_price);

      // ✅ Coin validation
      const availableCoins = parseFloat((data.customer_available_coins || '0').replace(/,/g, ''));
      const requiredCoins  = parseFloat((data.design_coin_use          || '0').replace(/,/g, ''));

      setAvailableAddicoins(data.customer_available_coins);
      setRequiredAddicoins(data.design_coin_use);

      // ✅ FIX: merged duplicate coin check
      ...
      setEstimateData({ ...estimatePayload, apiResponse: data });

      // ✅ FIX: only addicoins in form state — gst/discount removed
      setFieldValue('addicoins', data.design_coin_use || '0.00');

      setShowEstimateCard(true);
      setIsEstimateStale(false);
      setIsEstimateAccepted(false);

      if (isDesignSelf && isPrintSelf) setEstimateConform(true);

      // ✅ FIX: coupon fields included in orderPayload
      setOrderData({
        item_type:    'AK',
        item_code:    selectedItem,
        ...
        discount_per: couponData?.discount_percentage || 0,
        discount_amt: couponData?.discount_amount     || 0,
        addicoins:    data.design_coin_use || '0',
  ...
  // ─── Price card helper (reused across all 4 modes) ────────────────────────
  const PriceRows = ({ apiResponse }: { apiResponse: any }) => (
    <div className="space-y-3 text-sm">
      {parseFloat(apiResponse.design.replace(/,/g, '')) > 0 && (
        <li className="flex items-start gap-2">
          <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800 flex-shrink-0" />
          <div className="flex justify-between w-full">
            <span className="font-medium text-gray-700">Design</span>
            <span className="text-gray-700">₹{apiResponse.design}</span>
          </div>
        </li>
      )}
      {parseFloat(apiResponse.print.replace(/,/g, '')) > 0 && (
        <li className="flex items-start gap-2">
          <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800 flex-shrink-0" />
          <div className="flex justify-between w-full">
            <span className="font-medium text-gray-700">Print</span>
            <span className="text-gray-700">₹{apiResponse.print}</span>
          </div>
        </li>
      )}
      {(isAddiEase || isAddiEaseL) && estimateData?.laticess === 'Yes' && showLaticesField &&
        parseFloat(apiResponse.laticess.replace(/,/g, '')) > 0 && (
          <li className="flex items-start gap-2">
            <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800 flex-shrink-0" />
            <div className="flex justify-between w-full">
              <span className="font-medium text-gray-700">Latices</span>
              <span className="text-gray-700">₹{apiResponse.laticess}</span>
            </div>
          </li>
        )}
      {values.finish_type && parseFloat(apiResponse.finish.replace(/,/g, '')) > 0 && (
        <li className="flex items-start gap-2">
          <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800 flex-shrink-0" />
          <div className="flex justify-between w-full">
            <span className="font-medium text-gray-700">Finish</span>
            <span className="text-gray-700">₹{apiResponse.finish}</span>
          </div>
        </li>
      )}
      ...
      <li className="flex items-start gap-2">
        ...
        <div className="flex justify-between w-full">
          ...
          <span className="text-gray-700">₹{apiResponse.estimate_price}</span>
        </div>
      </li>
      {parseFloat(apiResponse.item_standard_discount.replace(/,/g, '')) > 0 && (
        <li className="flex items-start gap-2">
          <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800 flex-shrink-0" />
          <div className="flex justify-between w-full">
            <span className="font-medium text-gray-700">Standard Discount</span>
            <span className="text-gray-700">(-)₹{apiResponse.item_standard_discount}</span>
          </div>
        </li>
      )}
      {parseFloat(apiResponse.additional_discount.replace(/,/g, '')) > 0 && (
        <li className="flex items-start gap-2">
          <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800 flex-shrink-0" />
          <div className="flex justify-between w-full">
            <span className="font-medium text-gray-700">Additional Discount</span>
            <span className="text-gray-700">(-)₹{apiResponse.additional_discount}</span>
          </div>
        </li>
      )}
      {parseFloat(apiResponse.gst_18.replace(/,/g, '')) > 0 && (
        <li className="flex items-start gap-2">
          <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800 flex-shrink-0" />
          <div className="flex justify-between w-full">
            <span className="font-medium text-gray-700">GST (5%)</span>
            <span className="text-gray-700">₹{apiResponse.gst_18}</span>
          </div>
        </li>
      )}
      {parseFloat(apiResponse.gst_5.replace(/,/g, '')) > 0 && (
        <li className="flex items-start gap-2">
          <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800 flex-shrink-0" />
          <div className="flex justify-between w-full">
            <span className="font-medium text-gray-700">GST (5%)</span>
            <span className="text-gray-700">₹{apiResponse.gst_5}</span>
          </div>
        </li>
      )}
      <div className="border-t border-gray-200 my-2" />
      <div className="flex justify-between text-base font-bold text-primary">
        ...
        <span>₹{apiResponse.total_price}</span>
      </div>
    </div>
  );
  ...
  const TermsCheckbox = () => (
    <div className="mt-4 space-y-3">
      <div className="flex items-center space-x-2">
        ...
        />
        <Label htmlFor="enable-submit" className="text-sm font-medium leading-none ml-2">
          I agree to the{' '}
          <span
    ...
    <Card className="bg-gradient-to-br from-blue-10 to-indigo-50 shadow-sm border rounded-lg">
      ...
      <CardContent className="pt-2">
        <ul className="text-sm space-y-2.5">
          <li className="flex items-start gap-2">
            <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800" />
            <div className="flex justify-between w-full">
          ...
          {parseFloat(availableAddicoins?.replace(/,/g, '') || '0') -
            parseFloat(requiredAddicoins?.replace(/,/g, '') || '0') > 0 && (
              <li className="flex items-start gap-2">
                <div className="w-1.5 h-1.5 mt-2 rounded-full bg-purple-800" />
                <div className="flex justify-between w-full">
        ...
        <div className="flex-1 p-2 ml-5">
          ...
          <div className="space-y-4 mt-5">
                      ...
                      {finishOptions.map((option) => (
                        <label key={option.value} className="flex flex-col items-center cursor-pointer">
                          <input
                            ...
                            onChange={() => { setSelectedSubColor(null); setFieldValue('finish_type', option.value); }}
            ...
            {!orderId && !deviceTypeId && (
              <Button
                className="w-[350px] mt-10 py-6 bg-primary text-white font-semibold shadow-md"
            ...
            {showPreviwButton && (
              <Button
                className="w-[350px] mt-4 py-6 bg-primary text-white font-semibold shadow-md"
        ...
        {showInsufficientCoinsModal ? (
            ...
            <DialogContent className="sm:max-w-md">
              <DialogHeader className="text-center pb-2 pt-1">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <p className="font-semibold">Insufficient Addicoins</p>
                </div>
                <p className="text-sm text-gray-600 mt-2 text-center">
                  You do not have enough Addicoins to place this order. Please add more to proceed.
                </p>
              </DialogHeader>
              <div className="space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3 pt-2">
                  <Button variant="outline" onClick={() => setShowInsufficientCoinsModal(false)} className="flex-1">
                    Cancel
                  </Button>
                  <Button onClick={() => { setShowInsufficientCoinsModal(false); window.location.href = '/addicoins'; }} className="flex-1">
                    Add Addicoins
                  </Button>
                </div>
              </div>
          ...
          <div className="md:w-[500px] mr-40 space-y-4 mt-2">
            ...
            {!initialLoad && showEstimateCard && estimateData?.apiResponse && (() => {
              const ar = estimateData.apiResponse;

              // Self + Self → coins only
              if (isDesignSelf && isPrintSelf) return <CoinsCard />;

              // Self design + Addiwise print → coins + price card
              if (isDesignSelf && !isPrintSelf) return (
                <div className="flex flex-col gap-2">
                  ...
                  <Card className="bg-gradient-to-br from-blue-10 to-indigo-50 shadow-sm border rounded-lg">
                    ...
                    <CardContent className="pt-2">
                      <ul className="text-sm space-y-2.5">
                        <PriceRows apiResponse={ar} />
                      </ul>
                      {!isEstimateStale && <TermsCheckbox />}
                    </CardContent>
                  </Card>
                </div>
              );

              // Addiwise design + Addiwise print OR Addiwise design + Self print → price card only
              return (
