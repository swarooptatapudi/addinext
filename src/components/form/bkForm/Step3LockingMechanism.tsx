...
export const Step3 = ({
  ...
  const shouldShowError = (fieldName: string, isRequired = false) => {
    ...
    if (isRequired && (formSubmitted || touched[fieldName])) return !!errors[fieldName];
  ...
  useEffect(() => {
    const selectedSystem = values.locking_system;
    const matchingFiles = LOCAL_FILE_MAPPINGS[selectedSystem];

    if (selectedSystem && matchingFiles) {
      setLocalFiles({ locking_system: matchingFiles });

      setFieldValue('file_dimensions', matchingFiles[0] || '');
    } else {
      setLocalFiles({});
      setFieldValue('file_dimensions', '');
    }
  }, [values.locking_system, setFieldValue]);

  const handleLockingSystemChange = (value: string) => {
    handleChange('locking_system')(value);
  };

  const showFileUpload = values.locking_system === 'Other';
